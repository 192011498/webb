...
function onClickComplete() {
    ...
    str.push(`<div class="bill-item"> <div>Total Prices:</div> <div><b> ₹ ${grossAmount}</b></div></div>`)
...
function getImageString() {
    ...
    for (let i = 0; i < btqFlowers.length; i++) {
        ...
        if (!item.x) item.x = getRandomPixel(10, 70)
        if (!item.y) item.y = getRandomPixel(60, 170)
